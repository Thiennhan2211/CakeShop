...
function ProductCollection({
  ...
  const sizeOptions = StorefrontUtil.normalizeSizes(activeProduct?.sizes);
  const showCakeMessageInput = StorefrontUtil.shouldShowCakeMessageInput(activeProduct);

  const openQuickView = (product) => {
    setSizePickerProduct(null);
    setQuickViewProduct(product);
    setQuantity(1);
    setSelectedSize(StorefrontUtil.getDefaultSize(product));
    setCakeMessage('');
  };
  ...
  const handleAddToCart = (product, size, nextQuantity) => {
    const finalSize = size || StorefrontUtil.getDefaultSize(product);
    const productWithDynamicPrice = {
      ...product,
      price: StorefrontUtil.getPriceBySize(product, finalSize)
    };
          ...
          {products.map((product) => (
            <article key={product._id} className="store-product-card">
              <div className="store-product-card__media">
                <Link to={StorefrontUtil.buildProductPath(product)} className="store-product-card__image-link">
                  <img
              ...
              <div className="store-product-card__content">
                ...
                <h3>
                  <Link to={StorefrontUtil.buildProductPath(product)}>{product.name}</Link>
                </h3>
                <strong className="store-product-card__price">
                  {StorefrontUtil.formatCurrency(product.price)}
                </strong>
              </div>
            </article>
          ))}
          ...
          <div className="product-modal" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="product-modal__grid">
              <div className="product-modal__image">
                <img
                  src={`data:image/jpg;base64,${quickViewProduct.image}`}
              ...
              <div className="product-modal__info">
                ...
                <strong className="product-modal__price">
                  {StorefrontUtil.formatCurrency(StorefrontUtil.getPriceBySize(quickViewProduct, selectedSize))}
                </strong>
                <span className="product-modal__category">
                  {quickViewProduct.category?.name || 'Bộ sưu tập đặc biệt'}
                </span>
                <div className="product-modal__story">
                  {StorefrontUtil.buildProductStory(quickViewProduct).map((line) => (
                ...
                <div className="product-modal__actions">
                  <button
                    ...
                    onClick={() => openSizePicker(quickViewProduct)}
                  >
                    Chọn size
                  </button>
                  <button
                    ...
                    onClick={() => navigate(StorefrontUtil.buildProductPath(quickViewProduct))}
                  >
                    Xem chi tiết
                  </button>
                </div>
      ...
      {sizePickerProduct ? (
        <div className="product-modal-backdrop" onClick={closeModals}>
          <div className="product-modal product-modal--compact" onClick={(event) => event.stopPropagation()}>
            ...
            <strong className="product-modal__price">
              {StorefrontUtil.formatCurrency(StorefrontUtil.getPriceBySize(sizePickerProduct, selectedSize))}
            </strong>
